...
def main():
    if len(sys.argv) != 5:
        print("Usage: sandbox_runner.py <input_csv> <func_py> <output_csv> <col>", file=sys.stderr)
        sys.exit(1)
    ...
    if col not in df.columns:
        print(
            f"Column '{col}' not found in input CSV. Available: {list(df.columns)}", file=sys.stderr
        )
        sys.exit(1)
