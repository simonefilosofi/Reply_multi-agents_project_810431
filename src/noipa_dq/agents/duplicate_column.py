"""Detects exact and near-duplicate columns by pairwise similarity, keeps the member that agrees best with the rest of the table - measured, not assumed - fills its gaps from the siblings, and elects a canonical name per group. Similarity is measured on the string form and again on the numeric form, so columns differing only by zero-padding or storage type are still recognised. Validation reports follow the columns: those of dropped members are discarded and the survivor's are renamed. The election is deterministic whenever exactly one member already satisfies the naming convention; the LLM breaks ties or decides when none conforms, and its answer is normalised and validated before use."""
